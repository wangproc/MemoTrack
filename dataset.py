...
def _resolve_sportsmot_root(data_dir):
    candidates = [
        data_dir,
        os.path.join(data_dir, "dataset"),
        os.path.join(data_dir, "sportsmot_publish", "dataset"),
    ]
...
def get_mot_loader(
    ...
    if dataset == "mot17":
        ...
        if test:
            ...
            name = "train"
            annotation = "val_half.json"
    elif dataset in {"dance", "dancetrack"}:
        direc = "dancetrack"
        if test:
            name = "test"
            annotation = "test.json"
        ...
        direc = _resolve_sportsmot_root(data_dir)
        if test:
            name = "test"
            annotation = "test.json"
        else:
            name = "val"
            annotation = "val.json"
        ann_path = os.path.join(direc, "annotations", annotation)
        if not os.path.isfile(ann_path):
            raise RuntimeError(
                f"SportsMOT annotations not found at {ann_path}. "
                "Run prepare_sportsmot.py after extracting the official dataset."
            )
    else:
...
class MOTDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.seq_names = set(seq_names or [])
        ...
        if self.seq_names:
            self.ids = [
                ...
                if self.coco.loadImgs(img_id)[0]["file_name"].split("/")[0] in self.seq_names
            ]
