...
def canonical_dataset_name(dataset: str) -> str:
    low = dataset.lower()
    if low in {"dance", "dancetrack"}:
        return "dance"
    if low in {"sports", "sportsmot"}:
        return "sportsmot"
    return low
...
def get_detector_path_and_im_size(args) -> Tuple[str, Tuple[int, int]]:
    if getattr(args, "detector_path", ""):
        return args.detector_path, (800, 1440)
    ...
    if dataset == "mot17":
        ...
        size = (800, 1440)
    elif dataset == "sportsmot":
        detector_path = _first_existing_path(
            "external/weights/SportsMOT_yolox_x.tar",
            "/mnt/d/App/BiYing/SportsMOT_yolox_x.tar",
            "D:/App/BiYing/SportsMOT_yolox_x.tar",
        )
        size = (800, 1440)
    else:
...
class GeneralSettings:
    ...
    dataset_specific_settings: Dict[str, Dict[str, Union[float, bool, int]]] = {
        ...
        "dance": {"det_thresh": 0.5},
        "sportsmot": {"det_thresh": 0.5, "max_age": 50},
    }
    ...
    def max_age(seq_name: str) -> int:
        dataset_name = GeneralSettings.values.get('dataset', '')
        explicit = GeneralSettings.dataset_specific_settings.get(dataset_name, {}).get('max_age', None)
