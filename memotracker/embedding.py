...
class EmbeddingComputer:
    def __init__(self, dataset, test_dataset, grid_off, max_batch=1024):
        ...
        reid_override = os.environ.get("MEMOTRACK_REID_PATH", "").strip() or os.environ.get("BOOSTTRACK_REID_PATH", "").strip()
        reid_tag = Path(reid_override).stem if reid_override else ""
        extra_tag = os.environ.get("MEMOTRACK_EMB_NS", "").strip() or os.environ.get("BOOSTTRACK_EMB_NS", "").strip()
        suffix_parts = [part for part in [reid_tag, extra_tag] if part]
        suffix = "_" + "_".join(suffix_parts) if suffix_parts else ""
        self.cache_path = f"./cache/embeddings/{{}}_embedding{suffix}.pkl"
        self.cache = {}
        self.cache_name = ""
    ...
    def load_cache(self, path):
        ...
        if os.path.exists(cache_path):
            try:
                ...
                # Ignore truncated cache files left behind by interrupted runs.
                self.cache = {}
                try:
    ...
    def initialize_model(self):
        override_path = os.environ.get("MEMOTRACK_REID_PATH", "").strip() or os.environ.get("BOOSTTRACK_REID_PATH", "").strip()
        if override_path:
            model = FastReID(override_path)
            model.eval()
            model.cuda()
            model.half()
            self.model = model
        ...
        if self.dataset == "mot17":
            ...
            if self.test_dataset:
                ...
                return self._get_general_model()
        elif self.dataset in {"dance", "dancetrack"}:
            path = "external/weights/dance_sbs_S50.pth"
        elif self.dataset in {"sportsmot", "sports"}:
            candidates = [
                "external/weights/sports_sbs_S50.pth",
                "/mnt/d/App/BiYing/sports_sbs_S50.pth",
                "D:/App/BiYing/sports_sbs_S50.pth",
            ]
